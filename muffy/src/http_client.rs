...
mod tests {
    ...
    #[tokio::test]
    async fn resolve_headers_for_redirect() {
        let requests = Arc::new(Mutex::new(vec![]));
        let client = HttpClient::new(
            RecordingHttpClient {
                responses: [
    ...
    #[tokio::test]
    async fn resolve_max_age_for_redirect() {
        let foo_response = BareResponse {
            url: Url::parse("https://foo.com").unwrap(),
            status: StatusCode::MOVED_PERMANENTLY,
            headers: [(
        ...
        assert_eq!(
            ...
            .get(&foo_response.url, &|url| {
                Request::new(url.clone(), Default::default())
                    .set_max_age(if url.host_str() == Some("foo.com") {
                        CACHE_MAX_AGE
                    } else {
                        Default::default()
                    })
    ...
    async fn evict_cached_response_with_retry_status() {
        ...
        assert_eq!(
            ...
            .get(&url, &|url| request.clone().set_url(url.clone()))
            .await
            .unwrap(),
    ...
    #[tokio::test]
    async fn discard_stale_cache_on_failed_revalidation_after_period() {
        let url = Url::parse("https://foo.com").unwrap();
        let stale_response = BareResponse {
            url: url.clone(),
            status: StatusCode::OK,
        ...
        // The stale entry is not restored once it is past its period.
